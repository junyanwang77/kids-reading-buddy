...
- **`publish.sh`** follows the same archive/latest/index pattern as `daily-ai-news`'s `publish.sh`: dated files move into `archive/YYYY-MM/`, `latest.*` always point at the most recent publish, and `index.html` is regenerated from scratch each run (via `generate_index.py`, which scans `archive/*/day-*.html` for the latest date and a "往期内容" history list) rather than hand-edited.
