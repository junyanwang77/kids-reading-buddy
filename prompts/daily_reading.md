...
```json
{
  ...
  "vocabulary": [
    {"word": "单词", "pos": "词性缩写（n./v./adj./adv./prep./conj./pron./int. 等）", "meaning_cn": "中文意思", "example": "英文例句（最好取自故事原句）"}
  ],
...
2. **生词表**：这一步经常出问题，请严格按下面的标准执行：
   ...
   - 每个生词都要标注 pos（词性），优先参照 `data/cefrj-vocabulary-profile.csv` 里该词的 pos 列（同一个词可能有多个词性，选跟故事里实际用法一致的那个，比如 whole 在故事里是形容词就标 adj. 不是 noun）；词表里查不到的词（专有名词、复合词等）就根据它在句子里的实际用法自己判断词性。统一用英文缩写：n./v./adj./adv./prep./conj./pron./int.。
